...
MAX_QUERYIMAGE_COUNT = 10

basePathImg = "./Images/"
basePathQueryImg = "./examples/example_query/"
allDesc = {}
allFeatures = {}
allHistograms = {}
...
def Sift_and_Brisk():
    ...
    for i in range(1, MAX_QUERYIMAGE_COUNT + 1):
        sift_distances = {} 
        sift_allItems = ""
        qImg = extractGrayInstance(basePathQueryImg + str(i).zfill(2) +".jpg", F)
        for j in range(1, MAX_IMAGE_COUNT + 1):
            img = extractInstance(basePathImg + str(i).zfill(4) +".jpg", True)
            calcSiftBriskDist(qImg, img, i, j)

        sift_distances = {k: v for k, v in sorted(cnn_distances.items(), key=lambda item: item[1])}

        for key, d in sift_distances.items():
            sift_allItems += str(key) +" "
        toAppend = ("Q{}: {}".format(k, sift_allItems))
        f.write(toAppend)
        f.write("\n")
    f.close()
